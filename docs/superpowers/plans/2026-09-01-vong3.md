...
**Lưu ý:** `requireRole("CANDIDATE")` trong `lib/auth/session.ts` gọi `redirect()` nội bộ cho RECRUITER/ADMIN → redirect xảy ra tự động, không cần xử lý thêm. Không bọc `try/catch` vì sẽ catch mất redirect.
